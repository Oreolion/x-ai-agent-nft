...
const makeRequestSepolia = async () => {
  if (!process.env.ETHEREUM_PROVIDER) {
    throw new Error(
      "ETHEREUM_PROVIDER not provided - check your environment variables"
    );
  }
  if (!process.env.SUPABASE_API_KEY) {
    throw new Error(
      "SUPABASE_API_KEY not provided - check your environment variables"
    );
  }
  ...
  const routerAddress = "0xf9B8fc078197181C841c296C876945aaa425B278"; // Sepolia Router
  const donId = "fun-base-sepolia-1";
  const rpcUrl = process.env.ETHEREUM_PROVIDER; // fetch Sepolia RPC URL
